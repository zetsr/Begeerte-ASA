#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: UI_Button_ScreenshotNavigation

#include "Basic.hpp"

#include "UI_Button_ScreenshotNavigation_classes.hpp"
#include "UI_Button_ScreenshotNavigation_parameters.hpp"


namespace SDK
{

// Function UI_Button_ScreenshotNavigation.UI_Button_ScreenshotNavigation_C.BndEvt__UI_Button_ScreenshotNavigation_IMG_PrevImage_K2Node_ComponentBoundEvent_0_OnButtonClickedEvent__DelegateSignature
// (BlueprintEvent)

void UUI_Button_ScreenshotNavigation_C::BndEvt__UI_Button_ScreenshotNavigation_IMG_PrevImage_K2Node_ComponentBoundEvent_0_OnButtonClickedEvent__DelegateSignature()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("UI_Button_ScreenshotNavigation_C", "BndEvt__UI_Button_ScreenshotNavigation_IMG_PrevImage_K2Node_ComponentBoundEvent_0_OnButtonClickedEvent__DelegateSignature");

	UObject::ProcessEvent(Func, nullptr);
}


// Function UI_Button_ScreenshotNavigation.UI_Button_ScreenshotNavigation_C.Construct
// (BlueprintCosmetic, Event, Public, BlueprintEvent)

void UUI_Button_ScreenshotNavigation_C::Construct()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("UI_Button_ScreenshotNavigation_C", "Construct");

	UObject::ProcessEvent(Func, nullptr);
}


// Function UI_Button_ScreenshotNavigation.UI_Button_ScreenshotNavigation_C.ExecuteUbergraph_UI_Button_ScreenshotNavigation
// (Final, UbergraphFunction)
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UUI_Button_ScreenshotNavigation_C::ExecuteUbergraph_UI_Button_ScreenshotNavigation(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("UI_Button_ScreenshotNavigation_C", "ExecuteUbergraph_UI_Button_ScreenshotNavigation");

	Params::UI_Button_ScreenshotNavigation_C_ExecuteUbergraph_UI_Button_ScreenshotNavigation Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}


// Function UI_Button_ScreenshotNavigation.UI_Button_ScreenshotNavigation_C.SimulateDisabled
// (Public, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// bool                                    Disabled                                               (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UUI_Button_ScreenshotNavigation_C::SimulateDisabled(bool Disabled)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("UI_Button_ScreenshotNavigation_C", "SimulateDisabled");

	Params::UI_Button_ScreenshotNavigation_C_SimulateDisabled Parms{};

	Parms.Disabled = Disabled;

	UObject::ProcessEvent(Func, &Parms);
}

}

